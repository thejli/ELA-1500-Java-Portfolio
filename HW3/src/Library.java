...
public class Library {
	...
	public double lateFee(Person p) {
		...
		for(int q = 0; q < p.getCheckedOut().size(); q++) {
			...
			Calendar dueDate = new GregorianCalendar(dueYear, dueMonth, dueDay);
			
			if (current.after(dueDate)) {
				int daysBetween = (int)(current.getTime().getTime() - dueDate.getTime().getTime()) / (86400000);
				trumpTax += daysBetween * p.getCheckedOut().get(q).getBookValue() * .01;
			}
		}
		return trumpTax;
	}
